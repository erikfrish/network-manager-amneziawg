...
static size_t
mnl_ideal_socket_buffer_size(void)
{
...
static int
__mnl_attr_validate(const struct nlattr *attr,
...
static int
mnl_attr_parse(const struct nlmsghdr *nlh, unsigned int offset, mnl_attr_cb_t cb, void *data)
{
    int ret = MNL_CB_OK;
...
static bool
mnl_attr_put_check(struct nlmsghdr *nlh, size_t buflen, uint16_t type, size_t len, const void *data)
{
...
static int
mnl_cb_noop(__attribute__((unused)) const struct nlmsghdr *nlh, __attribute__((unused)) void *data)
{
    return MNL_CB_OK;
}

static int
mnl_cb_error(const struct nlmsghdr *nlh, __attribute__((unused)) void *data)
{
...
static int
mnl_cb_stop(__attribute__((unused)) const struct nlmsghdr *nlh, __attribute__((unused)) void *data)
{
    return MNL_CB_STOP;
}
...
static int
__mnl_cb_run(const void *buf, size_t numbytes, unsigned int seq, unsigned int portid, mnl_cb_t cb_data, void *data, const mnl_cb_t *cb_ctl_array, unsigned int cb_ctl_array_len)
{
...
static int
mnl_cb_run2(const void *buf, size_t numbytes, unsigned int seq, unsigned int portid, mnl_cb_t cb_data, void *data, const mnl_cb_t *cb_ctl_array, unsigned int cb_ctl_array_len)
{
...
__mnl_socket_open(int bus, int flags)
{
...
static int
mnlg_socket_send(struct mnlg_socket *nlg, const struct nlmsghdr *nlh)
{
    return mnl_socket_sendto(nlg->nl, nlh, nlh->nlmsg_len);
}

static int
mnlg_cb_noop(const struct nlmsghdr *nlh, void *data)
{
    (void)nlh;
    (void)data;
    return MNL_CB_OK;
...
static int
mnlg_cb_error(const struct nlmsghdr *nlh, void *data)
{
    const struct nlmsgerr *err = mnl_nlmsg_get_payload(nlh);
    (void)data;
...
static int
mnlg_cb_stop(const struct nlmsghdr *nlh, void *data)
{
    (void)data;
...
static int
mnlg_socket_recv_run(struct mnlg_socket *nlg, mnl_cb_t data_cb, void *data)
{
    int err;
...
static int
get_family_id_attr_cb(const struct nlattr *attr, void *data)
{
...
static int
get_family_id_cb(const struct nlmsghdr *nlh, void *data)
{
    uint16_t *p_id = data;
...
static struct mnlg_socket *
mnlg_socket_open(const char *family_name, uint8_t version)
{
...
static void
mnlg_socket_close(struct mnlg_socket *nlg)
{
    mnl_socket_close(nlg->nl);
    free(nlg->buf);
    free(nlg);
...
static int
parse_linkinfo(const struct nlattr *attr, void *data)
{
...
static int
parse_infomsg(const struct nlattr *attr, void *data)
{
    struct interface *interface = data;
...
static int
read_devices_cb(const struct nlmsghdr *nlh, void *data)
{
...
static int
fetch_device_names(struct string_list *list)
{
...
static int
add_del_iface(const char *ifname, bool add)
{
    struct mnl_socket *nl = NULL;
    char *rtnl_buffer;
    ssize_t len;
...
static int
parse_allowedip(const struct nlattr *attr, void *data)
{
...
static int
parse_allowedips(const struct nlattr *attr, void *data)
{
    wg_peer *peer = data;
...
static int
parse_peer(const struct nlattr *attr, void *data)
{
    wg_peer *peer = data;
...
static int
parse_peers(const struct nlattr *attr, void *data)
{
...
static int
parse_device(const struct nlattr *attr, void *data)
{
    wg_device *device = data;
...
static int
read_device_cb(const struct nlmsghdr *nlh, void *data)
{
    return mnl_attr_parse(nlh, sizeof(struct genlmsghdr), parse_device, data);
}

static void
coalesce_peers(wg_device *device)
{
...
int
wg_get_device(wg_device **device, const char *device_name)
{
    int ret = 0;
    struct nlmsghdr *nlh;
    struct mnlg_socket *nlg;
...
int
wg_add_device(const char *device_name)
{
    return add_del_iface(device_name, true);
}

int
wg_del_device(const char *device_name)
{
    return add_del_iface(device_name, false);
}

void
wg_free_device(wg_device *dev)
{
    wg_peer *peer, *np;
